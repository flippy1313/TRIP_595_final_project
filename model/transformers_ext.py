...
class ClassificationHead(nn.Module):
    ...
    def forward(self, features, precond=None, return_embeddings=False):
...
class TieredModelPipeline(nn.Module):
  def __init__(self, embedding, num_sents, num_attributes, labels_per_att, config_class, model_name, device, ablation=[], precondition_aware=False, cross_attn=False, loss_weights=[0.0, 0.4, 0.4, 0.2, 0.0]): # labels_per_att is a dictionary mapping attribute index to number of labels
    super().__init__()
    ...
    if 'states' not in ablation:
      self.states_proj = nn.Linear(66, embedding_proj_size) ##MARK: Please Comment this line and Uncomment these lines below
      # if cross_attn:
      #   self.states_proj = nn.Linear(66, embedding_proj_size)
      # else:
      #   self.states_proj = nn.Linear(132, embedding_proj_size)
    if 'states-attention' in ablation:
      self.states_attention = nn.Sequential(nn.Linear(embedding_proj_size, embedding_proj_size), nn.Sigmoid()) # Attention mechanism conditioned on states
    ...
    #self.precondition_aware = precondition_aware
    
    ##MARK: please uncomment the above lines
    ##ADDED FOR CROSS ATTENTION
    self.k = nn.Linear(66,66)
    self.q = nn.Linear(66,66)
    self.v = nn.Linear(66,66)
    self.crossattn = nn.MultiheadAttention(66, 2, 0.1)
  ...
  def forward(self, input_ids, input_lengths, input_entities, attention_mask=None, token_type_ids=None, attributes=None, preconditions=None, effects=None, conflicts=None, labels=None, training=False):
    ...
    for i in range(self.num_attributes):
      out_s, precond_emb = self.precondition_classifiers[i](out, return_embeddings=True)
      precond_for_effect.append(precond_emb)
    ...
    # print(out.shape, out_preconditions.shape, out_effects.shape)
    for i in range(self.num_attributes):
      # if self.precondition_aware:
      # out_s = self.effect_classifiers[i](out, precond=precond_for_effect[i], return_embeddings=False)
      # else:
      #   out_s = self.effect_classifiers[i](out, return_embeddings=False)
      out_s = self.effect_classifiers[i](out, return_embeddings=False) ##MARK: please comment this line and uncomment the above lines
      with torch.no_grad(): # Don't allow backprop from conflict detection to state classifiers
    ...
    # print("q,k,v,crossattn:", out_preconditions.shape,  out_effects.shape)
    # print("out:", out.shape)
    
    out_cross_attn = None
    # if self.cross_attn:
    #   q = self.q(out_effects)
    #   k = self.k(out_effects)
    #   v = self.v(out_preconditions)
    #   out_cross_attn = self.crossattn(q,k,v)[0]
    q = self.q(out_effects)##MARK: please comment this line and uncomment the above lines
    k = self.k(out_effects) ##MARK: please comment this line
    v = self.v(out_preconditions) ##MARK: please comment this line
    out_cross_attn = self.crossattn(q,k,v)[0] ##MARK: please comment this line
    ...
    #print("out_state_after_cat", out_states.shape)
    if 'states' not in self.ablation:
      # if self.cross_attn:
      #   out_states = self.dropout(out_cross_attn)
      out_states = self.dropout(out_cross_attn)##MARK: please comment this line and uncomment the above lines
      out_states = self.dropout(out_states)
      out_states = self.states_proj(out_states)
      #print('state:',out_states.shape)
    #print("out_states_after_proj", out_states.shape)
    if 'states-attention' in self.ablation:
      #print('state-attn:',out.shape)
      #print('state-attn:',out_states.shape)
    ...
    #   if loss_conflicts >= max(loss_preconditions / self.num_attributes, loss_effects / self.num_attributes, loss_conflicts):
    #     self.loss_weights[3] = self.loss_weights[3] + 0.02
    #   elif loss_effects / self.num_attributes >= max(loss_preconditions / self.num_attributes, loss_effects / self.num_attributes, loss_conflicts):
    #     self.loss_weights[2] = self.loss_weights[2] + 0.02
    #   elif loss_preconditions / self.num_attributes >= max(loss_preconditions / self.num_attributes, loss_effects / self.num_attributes, loss_conflicts):
    #     self.loss_weights[1] = self.loss_weights[1] + 0.02

    #   if loss_conflicts <= min(loss_preconditions / self.num_attributes, loss_effects / self.num_attributes, loss_conflicts):
    #     self.loss_weights[3] = self.loss_weights[3] - 0.02 
    #   if loss_effects / self.num_attributes <= min(loss_preconditions / self.num_attributes, loss_effects / self.num_attributes, loss_conflicts):
    #     self.loss_weights[2] = self.loss_weights[2] - 0.02
    #   if loss_preconditions / self.num_attributes <= min(loss_preconditions / self.num_attributes, loss_effects / self.num_attributes, loss_conflicts):
    #     self.loss_weights[1] = self.loss_weights[1] - 0.02

    #   if self.loss_weights[3] < 0:
    #     self.loss_weights[3] = 0
    ...
    if loss_preconditions is not None:# and not self.freeze: ##MARK Please uncomment this
      total_loss += self.loss_weights[1] * loss_preconditions / self.num_attributes
    if loss_effects is not None:# and not self.freeze: ##MARK Please uncomment this
