...
def train_epoch_tiered(model, freeze, dynamic_weight, optimizer, train_dataloader, device, epoch, seg_mode=False, return_losses=False, build_learning_curves=False, val_dataloader=None, train_lc_data=None, val_lc_data=None):
  t0 = time.time()
  ...
  # if epoch > 10:
  #   model.freeze = 2
  #   for index in range(len(model.precondition_classifiers)):
  #     for param in model.precondition_classifiers[index].parameters():
  #       param.requires_grad = False
  #   for index in range(len(model.effect_classifiers)):
  #     for param in model.effect_classifiers[index].parameters():
  #       param.requires_grad = False  
  ...
  bar = progressbar.ProgressBar(maxval=bar_size, widgets=[progressbar.Bar('#', '[', ']'), ' ', progressbar.Percentage()])
  bar_idx = 0
  bar.start()
